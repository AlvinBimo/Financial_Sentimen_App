...
def display_dataframe(
    ...
    st.dataframe(df, use_container_width=True, column_config= {"link" : st.column_config.LinkColumn()})
...
def display_articles(df: pd.DataFrame) -> None:
    ...
    st.sidebar.markdown("---")
    capped_count = min(st.session_state.user_feedback_count, MIN_FEEDBACK_REQUIRED)
    ...
    with article_options_col2:
        highlight_relevant_sentence = st.checkbox("Highlight Relevant Sentence", value=True)
...
st.markdown("<h1 style='text-align: center;'>Jamkrindo Sentiment Analysis</h1>", unsafe_allow_html=True)
...
# Buttoon untuk mengumpulkan data dan melakukan analisis
if 'user_click' not in st.session_state:
    st.session_state.user_click = 0

st.session_state.button_disabled = st.session_state.get('button_disabled', False)
...
if start_button:
    ...
    try:
        with st.spinner("Mengumpulkan artikel dan melakukan analisis", show_time=True):
            df = collect_data_and_analyze_sentiment(
                SENTIMENT_FILE_PATH, selected_news, max_items, query,
                start_date, end_date
            )
            ...
            # ----- decide minimum feedback -----
            if df.empty:
                st.session_state.button_disabled = False
            else:
                MIN_FEEDBACK_REQUIRED = min(len(df), 5)

            # ----- enforce rule only on repeat clicks -----
            if not df.empty and st.session_state.user_click > 1 and \
               st.session_state.user_feedback_count < MIN_FEEDBACK_REQUIRED:
        ...
        st.rerun()          # <― only one rerun, at the very end
